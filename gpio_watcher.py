#!/usr/bin/env python3
"""Watches GPIO lines per bindings.json and fires Companion HTTP API actions."""
import json
import sys
import time
import urllib.parse
...
def run_action(binding, event_type):
    ...
    try:
        if kind == "press":
            ...
            http_post(url)
            log(f"GPIO{binding['bcm']} {event_type} -> press {a['page']}/{a['row']}/{a['column']}")
        elif kind == "down_up":
            sub = "down" if event_type == "falling" else "up"
            url = f"{COMPANION}/api/location/{a['page']}/{a['row']}/{a['column']}/{sub}"
            http_post(url)
            log(f"GPIO{binding['bcm']} {event_type} -> {sub} {a['page']}/{a['row']}/{a['column']}")
        elif kind == "variable":
            ...
            log(f"GPIO{binding['bcm']} {event_type} -> variable {a['variable']}={val}")
    except Exception as e:
        log(f"action error on GPIO{binding.get('bcm')}: {e}")
